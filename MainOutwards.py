...
@author: Anetoh Stanley
'''
import os, time
import uuid
import json as sj
import shutil
import modules.businesslogic.XMLRequestParser as xmlparser
from modules.businesslogic.ItemsLogic import ItemsLogic
import modules.businesslogic.SqlAlchemy as alchemy
import modules.businesslogic.ItemsLogic as itemlogic
from modules.businesslogic.LovLogic import LovLogic
import modules.rabbitmq.message_sender as msg_sender
from modules.businesslogic.FormsLogic import FormsLogic
...
import datetime
from modules.amt2word.num2word_EN import Num2Word_EN
import Image
...
logic = LovLogic()
rootdir = logic.getAchOutDir() #'C:/ach_files/in/'#
xrootdir = logic.getAchOutDir() + 'express\\' #'C:/ach_files/in/'#
routeno = logic.getBankRouteNo()
imgtempdir = logic.getInwardTempImgDir()

outwardbackup = logic.getOutwardFileBackUpDir()




def getListOfRequest():
    retlist = []
    filterResult = logic.executeQuery("select gendate, filetype from file_gen_request where generated = 'N'")
    for row in filterResult:
        retlist.append({'gendate' : str(row[0]), 'filetype' : str(row[1])})
    return retlist

def updateRequest(date_created, filetype):
    #logic.executeInsrt("update file_gen_request set generated = 'Y' where gendate = '%s' and filetype = '%s'" % (date_created, filetype))
    output = ''
    try:
        logic.executeInsrt("update file_gen_request set generated = 'Y' where gendate = '%s' and filetype = '%s'" % (date_created, filetype))
    except Exception, e:
...
def startSystem():
    print('Initializing Outward Service....')
    print('Service Started!')

    while 1:

        try:
            for daterow in getListOfRequest():
                if daterow:

                    date_created = daterow['gendate']#str(datetime.date.today())[0:4] + '-' + str(datetime.date.today())[5:7] + '-' + str(datetime.date.today())[8:10]
                    filetype = daterow['filetype']
                    print filetype
                    print date_created
                    if filetype == 'outward': #Outward file generation
                        dbg(routeno)
                        generateOutgoingFiles(date_created, routeno)
                        updateRequest(date_created, filetype)

                    elif filetype == 'express': #Outward file generation
                        generateOutgoingFiles2(date_created, routeno)
                        updateRequest(date_created, filetype)

                    elif filetype == 'entries': #Outward entries generation
                        generateOutGoingGefuEntries(date_created)
                        updateRequest(date_created, filetype)

                    elif filetype == 'expentries': #Outward entries generation
                        generateOutGoingGefuEntriesexp(date_created)
                        updateRequest(date_created, filetype)

                    elif filetype == 'outrejentries': #Outward reject reversal entries
                        generateOutwardRejectReversalGefuEntries(date_created)
                        updateRequest(date_created, filetype)

                    elif filetype == 'exprejentries': #Outward reject reversal entries
                        generateOutwardRejectReversalGefuEntriesexp(date_created)
                        updateRequest(date_created, filetype)

                    '''elif filetype == 'inrejentries': #Outward reject reversal entries
                        generateInwardRejectGefuEntries(date_created)
                        updateRequest(date_created, filetype)'''
        except Exception, e:
            logger.error(e)
...
def getNextFileID():
    ...
    try:
        logic = LovLogic()
        filterResult = logic.executeQuery("select  cts_fileid_seq.nextval from dual")
        for row in filterResult:
